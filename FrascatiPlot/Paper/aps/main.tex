...
The formation of light anti- and hyper-nuclei in high energy proton-proton (pp), proton-nucleus (pA) and nucleus-nucleus (AA) collisions provides unique observables for the study of the system created in these collisions.
Nuclei and hyper-nuclei are special objects with respect to non-composite hadrons (pions, kaons, protons, etc.), because their size is comparable to a fraction or the whole system created in the collision~\cite{Adam:2015vna}.  
...
For the hyper-triton, theoretical calculations indicate a rms of the wave-function of about 5 fm \cite{Nemura:1999qp}, which is significantly larger than that of non-strange nuclei with mass number $A = 3$, driven by the average separation of the $\Lambda$ with respect to the two other nucleons.

\input{table1}

For about sixty years, coalescence models have been used to describe the formation of composite objects (see for instance \cite{Butler:1963, Kapusta:1980, Sato:1981ez, Nagle:1996vp, Scheibl:1998tk, Cho:2017dcy, Blum:2017qnn, Bazak:2018hgl, Zhao:2018lyf} and references therein).
Surprisingly, thermal-statistical models have been successful in describing also the production of light (anti-)(hyper-)nuclei across a wide range of energies in nucleus-nucleus collisions \cite{Andronic:2010qu,Andronic:2017}. 
...
Based on our findings, we propose that \bA~is systematically measured in all collision systems by exploiting the large statistics sample foreseen at the LHC Run 3 \& 4, in order to discriminate the aforementioned scenarios.
...
%
The LHC is particularly suited for the production of anti-nuclei, since the number of baryons and anti-baryons is essentially equal at midrapidity \cite{Abbas:2013rua}. As a consequence, the anti-particle to particle ratio for (hyper-)nuclei is measured to be consistent with unity \cite{ALICE:nucleipp2017, anielski-HQ14, Acharya:2017dmc, Adam:2015yta}.
...
The quantum-mechanical nature of the coalescence products is explicitly accounted for by means of an average correction factor, $\langle C_{A} \rangle$. In the case of the deuteron, the quantum-mechanical correction factor $\langle C_{d} \rangle$ has been approximated as 
...
\end{equation}
%
which is the main result of \cite{Scheibl:1998tk}. The coalescence parameter decreases with increasing volume, as expected. 
The quantum-mechanical correction factor introduces a length scale defined by the deuteron size relative to the source size. 
...
In particular, for the case of \hethree~with $A=3$ and $J=1/2$, the latter becomes the Eq. 9 presented in~\cite{Blum:2017qnn}.

In summary, under the assumption $R_1\approx R_2 \approx R_3 \approx R$ as in \cite{Blum:2017qnn} (see details in next section) and by combining Eq. 6.2 in \cite{Scheibl:1998tk} and Eq.~\ref{eq:CA_general}, we obtain the following general expression for the coalescence parameter:
...
This volume is experimentally accessible with Hanbury-Brown-Twiss (HBT) interferometry. 
...
In contrast to \cite{Blum:2017qnn}, we do not explicitly use the measured HBT radii in our study, but we derive the radii from the measured \avdNdeta~according to the following relation:
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=.8\columnwidth]{../HbtRadiusParam.png}
...
Therefore, \pt~spectra have been modelled using a blast-wave~\cite{Schnedermann:1993ws} parameterization. When extracting the predicted spectrum for a given particle species (proton, $\Lambda$, deuteron, \hethree), the parameters of the blast-wave 
are fixed to the values obtained from the simultaneous fit to the pion, kaon and proton spectra measured in Pb--Pb collisions as a function of centrality by ALICE~\cite{Abelev:2013vea}. 
...
In the case of hyper-triton, the normalisation is extracted from the statistical-thermal model prediction of the strangeness population factor $S_{3} = { _{\Lambda}^{3}\mathrm{H} / ^{3}{\mathrm{He}} \over \Lambda/p}$ multiplied by the measured $\Lambda/p$ ratio~\cite{Abelev:2013vea,Abelev:2013xaa} and \hethree~yield~\cite{ALICE:deuteronppPbPb2015}. Based on the spectra obtained in this way, we calculate the corresponding coalescence parameters for a given \pt/$A$ and compare it with coalescence expectations. 
...
Data on anti- and hyper-nuclei production at the LHC in different collision systems have been released by the ALICE Collaboration in centrality/multiplicity classes \cite{ALICE:nucleipp2017,ALICE:deuteronppPbPb2015,Acharya:2017dmc, Adam:2015yta}. 
To facilitate the comparison with multiplicity-dependent data given in the INEL$>$0 class, we have rescaled the data in \cite{ALICE:nucleipp2017}, given for inelastic pp collisions, by the ratio of \avdNdeta~in these two event classes \cite{Adam:2015gka}.
In order to map the \avdNdeta~to the source size we first used the parameterisation (labeled as ``A'') from the fit to the HBT data shown in Fig.~\ref{fig:radiiparam}. 
We found a discrepancy between the coalescence volume from parameterisation A and the curve from the coalescence calculation. In particular, the model would require a larger radius for a given value of \btwo~in Pb--Pb collisions.
In a second step we tuned the parameterisation (labeled as ``B'') such that the data points for (anti-)deuterons fall onto the coalescence prediction, finding the values $a = 0.473$ and $b = 0$ for the parameters of Eq.~\ref{eq:radiusParameterisation}. 

With parameterisation B, we investigate the agreement of the model with the measured \bthree\ for \hethree. Thus the coalescence volume is constrained with the more differential (anti-)deuteron data assuming that the coalescence volume is the same for all anti- and hyper-nuclei. 
The necessity of introducing a second parameterisation for the radius might suggest that the kinetic freeze-out volume relevant for coalescence is not given by the $m_{T}$-scaled pion HBT volume considered in parameterisation A, but by a slightly larger system as from parameterisation B.  
