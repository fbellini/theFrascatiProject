...
void CoalescenceBA(Double_t pToA = 0.75)
{
  ...
  Short_t nucleiA[8] = {2, 3, 3, 3, 4, 4, 4, 4};
  ...
  TH2D * hframe4 = new TH2D("hframe4Fig4", "B_{A} vs radius; #it{R} (fm); #it{B}_{A}", 1000, 0.01, 6.0, 2000, 1.e-12, 1.e-1);
  ...
  hframe4->GetXaxis()->SetRangeUser(0.01, 6.);
...
TGraphErrors * MakeBATheoryGraphCoalescence(Short_t A, Double_t JA, Double_t mT, Double_t objSize)
{
  ...
  graphOut->SetTitle(Form("B_{%i} from coalescence", A));
  
  
  for (int i = 0; i<nPoints; i++){
    gR[i] = 10.0 * i / nPoints; 
    gY[i] = getBAfromRadius(A, JA, mT, gR[i], objSize);
